...
export class Equipment extends Component {
	componentDidMount() {
		this.getEquipment(this.props.budgetYear)
		// To disabled submit button at the beginning.
		this.props.form.validateFields()
	}

	getEquipment = async budgetYear => {
		try {
			...
			console.log(error)
			message.error('Failed to load the equipment data!')
		}
	}
	...
	handleSubmit = e => {
		...
		this.props.form.validateFields(async (err, values) => {
			if (!err) {
				...
				console.log('Received values of Equipment form: ', values)
				const voucherExist = await checkVoucher('equipment', values.voucher)
				if (voucherExist) {
				...
				console.log('Equipment form data formated: ', data)
				axios
					.post('/api/v1/equipment', data)
					.then(response => {
						const data = response.data
						console.log('Equipment form submit response: ', data)
						/* Instant UI update */
						this.addData(data)
						this.setState({ working: false })
		...
		return (
			<>
				<Form layout="inline" onSubmit={this.handleSubmit}>
					...
					<Form.Item validateStatus={equipmentError ? 'error' : ''} help={equipmentError || ''}>
						{getFieldDecorator('equipment', {
							rules: [{ required: true, message: 'Please provide Equipment Type!' }]
						})(<Input placeholder="Equipment Type" style={{ minWidth: 300 }} />)}
					</Form.Item>
...
const EquipmentForm = Form.create({ name: 'equipment_form' })(Equipment)
...
class EditableCell extends React.Component {
	getInput = (field, getFieldDecorator, title, record) => {
		switch (field) {
			...
			case 'equipment':
				return getFieldDecorator('equipment', {
					initialValue: record[field],
					rules: [{ required: true, message: 'Please provide Equipment Type!' }]
				})(<Input placeholder="Equipment Type" style={{ minWidth: 200 }} />)
...
class EditableTable extends React.Component {
	...
	save(form, key) {
		form.validateFields((err, row) => {
			if (!err) {
				console.log('Received values of Equipment Update form: ', row)
				const data = {
					voucher: row.voucher,
				...
				console.log('Equipment updated form data formated: ', data)
				/* Instant UI update */
				this.props.updateData(key, data)
				axios
					.post(`/api/v1/equipment/${key}`, data)
					.then(response => {
						const data = response.data
						console.log('Equipment form update response: ', data)
						// To disabled submit button
						this.props.form.validateFields()
						this.cancel(key)
